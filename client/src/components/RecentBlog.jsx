...
const RecentBlog = ({ blog }) => {
  ...
  return (
    <Link to={`/blog/${blog.slug}`} className="group block w-full h-full">
      <div
        className={`h-full flex flex-col rounded-tl-2xl rounded-br-2xl border overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ${
      ...
      >
        ...
        <div className="flex flex-col gap-3 px-4 py-4 flex-1">
          <p
            className={`text-base md:text-lg font-semibold line-clamp-2 min-h-[3.25rem] transition-colors ${
              isDark
          ...
          <span
            className={`text-xs font-medium w-fit px-4 py-1 rounded-full border mt-auto ${
              isDark
